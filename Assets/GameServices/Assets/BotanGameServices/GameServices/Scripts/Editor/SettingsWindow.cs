...
using BotanGameServices.GameServices.Internal;
using System;
    ...
    public class SettingsWindow : EditorWindow
    {
        ...
        private void SaveSettings()
        {
            ...
            switch (step)
            {
                ...
                case 1:
                    BotanGameServices.Main.EditorUtilities.CreateFolder($"{rootFolder}/Plugins/Android/");
                ...
                case 5:
                    if (useForAndroid)
                    {
                        PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BotanGameServices_GAMESERVICES_ANDROID, false, BuildTargetGroup.Android);
                    }
                    else
                    {
                        PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BotanGameServices_GAMESERVICES_ANDROID, true, BuildTargetGroup.Android);
                    }
                    if (useForIos)
                    {
                        PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BotanGameServices_GAMESERVICES_IOS, false, BuildTargetGroup.iOS);
                    }
                    else
                    {
                        PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BotanGameServices_GAMESERVICES_IOS, true, BuildTargetGroup.iOS);
                    }
                    step++;
                    installing = true;
                    break;
        ...
        private void CreateEnumFiles()
        {
            ...
            string text =
            "namespace BotanGameServices.GameServices\n" +
            "{\n" +
            ...
            text += "\t}\n";
            text += "}";
            File.WriteAllText($"{Application.dataPath}/{rootWithoutAssets}/Scripts/Public/AchievementNames.cs", text);
            ...
            text += "\t}\n";
            text += "}";
            File.WriteAllText($"{Application.dataPath}/{rootWithoutAssets}/Scripts/Public/LeaderboardNames.cs", text);
