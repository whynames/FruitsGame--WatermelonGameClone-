...
namespace BotanGameServices.GameServices.Internal
{
    public class GameServicesTest : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            if (!BotanGameServices.GameServices.API.IsLoggedIn())
            {
                if (Button("Login"))
                {
                    //Login to Game Services
                    BotanGameServices.GameServices.API.LogIn(LoginResult);
                }
            }
            else
            {
                if (allAchievements.Length > 0)
                {
                    ...
                    if (Button("Submit Selected Achievement"))
                    {
                        ...
                        BotanGameServices.GameServices.API.SubmitAchievement(allAchievements[indexNumberAchievements], AchievementSUbmitted);
                ...
                if (allLeaderboards.Length > 0)
                {
                    ...
                    if (Button("Submit Score"))
                    {
                        ...
                        BotanGameServices.GameServices.API.SubmitScore(score, allLeaderboards[indexNumberLeaderboards], ScoreSubmitted);
