...
namespace BotanGameServices.IAPSystem.Local
{
    ...
    {
        public class DefinedStoreProducts
        {
            public ShopProductNames name;
            public bool bought;

            public DefinedStoreProducts(ShopProductNames name, bool bought)
            {
                this.name = name;
                this.bought = bought;
            }
        ...
        private List<DefinedStoreProducts> consumableProducts = new List<DefinedStoreProducts>();
        private List<DefinedStoreProducts> nonCOnsumableProducts = new List<DefinedStoreProducts>();
        private List<DefinedStoreProducts> subscriptions = new List<DefinedStoreProducts>();
        ...
        void OnGUI()
        {
            ...
            if (showButtons)
            {
                if (!BotanGameServices.IAPSystem.CodeBase.IsInitialized())
                {
                    if (initializationInProgress == false)
                    {
                        if (Button("Initialize"))
                        {
                            ...
                            //Initialize IAP
                            BotanGameServices.IAPSystem.CodeBase.Initialize(InitializeResult);
                        }
                    }
                }
                else
                {
                    if (purchaseInProgress == false)
                    {
                        ...
                        if (consumableProducts.Count > 0)
                        {
                            ...
                            if (Button("BUY " + consumableProducts[indexNumberConsumable].name + " " + BotanGameServices.IAPSystem.CodeBase.GetValue(consumableProducts[indexNumberConsumable].name) + " Coins " + BotanGameServices.IAPSystem.CodeBase.GetLocalizedPriceString(consumableProducts[indexNumberConsumable].name)))
                            {
                                purchaseInProgress = true;

                                //this function can be called like this
                                //IAPManager.Instance.BuyProduct(ShopProductNames.Consumable1, ProductBought);
                                BotanGameServices.IAPSystem.CodeBase.BuyProduct(consumableProducts[indexNumberConsumable].name, ProductBought);
                            }
                        }
                        else
                        {
                        ...
                        if (nonCOnsumableProducts.Count > 0)
                        {
                            ...
                            if (Button("BUY " + nonCOnsumableProducts[indexNumberNonConsumable].name + " " + BotanGameServices.IAPSystem.CodeBase.GetLocalizedPriceString(nonCOnsumableProducts[indexNumberNonConsumable].name)))
                            {
                                purchaseInProgress = true;

                                //this function can be called like this
                                //IAPManager.Instance.BuyProduct(ShopProductNames.NonConsumable1, ProductBought);
                                BotanGameServices.IAPSystem.CodeBase.BuyProduct(nonCOnsumableProducts[indexNumberNonConsumable].name, ProductBought);
                            }
                        }
                        else
                        {
                        ...
                        if (subscriptions.Count > 0)
                        {
                            ...
                            if (Button("BUY " + subscriptions[indexNumberSubscription].name + " " + BotanGameServices.IAPSystem.CodeBase.GetLocalizedPriceString(subscriptions[indexNumberSubscription].name)))
                            {
                                purchaseInProgress = true;

                                //this function can be called like this
                                //IAPManager.Instance.BuyProduct(ShopProductNames.Subscription1, ProductBought);
                                BotanGameServices.IAPSystem.CodeBase.BuyProduct(subscriptions[indexNumberSubscription].name, ProductBought);
                            }
                        }
                        else
                        {
                        ...
                        {
                            BotanGameServices.IAPSystem.CodeBase.RestorePurchases(ProductBought, RestoreDone);
                        }
        ...
        private void InitializeResult(IAPOperationStatus status, string message, List<StoreProduct> shopProducts)
        {
            ...
            consumableProducts = new List<DefinedStoreProducts>();
            nonCOnsumableProducts = new List<DefinedStoreProducts>();
            subscriptions = new List<DefinedStoreProducts>();
                ...
                for (int i = 0; i < shopProducts.Count; i++)
                {
                    ...
                    switch (shopProducts[i].productType)
                    {
                        case ProductType.Consumable:
                            consumableProducts.Add(new DefinedStoreProducts(BotanGameServices.IAPSystem.CodeBase.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                        case ProductType.NonConsumable:
                            nonCOnsumableProducts.Add(new DefinedStoreProducts(BotanGameServices.IAPSystem.CodeBase.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                        case ProductType.Subscription:
                            subscriptions.Add(new DefinedStoreProducts(BotanGameServices.IAPSystem.CodeBase.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                            break;
                    }
                }

                //UnityEngine.Purchasing.SubscriptionInfo info = IAPManager.Instance.GetSubscriptionInfo(ShopProductNames.Subscription);
                //BotanGameServicesEasyIAP.ScreenWriter.Write("GET INFO:");
                //if (info != null)
                //{
                //    BotanGameServicesEasyIAP.ScreenWriter.Write(info.getExpireDate());
                //}
            }
            else
            {
        ...
        //custom drop down
        void DropDown(int maxButtons, ref int indexNumber, ref bool showDropDown, List<DefinedStoreProducts> coll)
        {
