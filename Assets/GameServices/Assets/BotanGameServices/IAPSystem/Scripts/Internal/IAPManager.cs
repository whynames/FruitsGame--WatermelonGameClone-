    ...
    public class IAPManager : MonoBehaviour, IDetailedStoreListener
    {
        ...
        public static IAPManager Instance
        {
            get
            {
                if (instance == null)
                {
                    GameObject go = new GameObject("BotanGameServicesIAPManager");
        ...
        {
            IAPSystemData settings = Resources.Load<IAPSystemData>(Constants.DATA_NAME_RUNTIME);
            if (settings == null)
            {
                Debug.LogError("No products available -> Go to Tools->BotanGameServices->Easy IAP and define your products");
                return;
            }
            shopProducts = settings.shopProducts;
            debug = settings.debugIAP;
...
#if BOTANGAMESERVICES_IAP_VALIDATION
        ...
        public ProductType GetProductType(ShopProductNames product)
        {
            ...
            {
                Debug.LogError("Not Initialized -> Call BotanGameServices.EasyIAP.API.Initialize() before anything else");
                return 0;
            }
        }
        ...
        public string GetLocalizedPriceString(ShopProductNames product)
        {
            if (IsInitialized())
            {
                ...
                {
                    Debug.LogError("No products available -> Go to Window->BotanGameServices->Easy IAP and define your products");
                }
            }
            else
            {
