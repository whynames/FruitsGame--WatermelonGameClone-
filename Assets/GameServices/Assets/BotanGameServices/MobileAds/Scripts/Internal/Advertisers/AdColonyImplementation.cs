    ...
    internal class AdColonyImplementation : MonoBehaviour, IAdProvider
    {
        ...
        public void InitializeAds(PlatformSettings platformSettings, UnityAction onInitialized)
        {
            if (initialized == false)
            {
                ...
                //verify settings
                BotanGameServicesLogger.AddLog($"{settings.appId.displayName} : {appId}");
                BotanGameServicesLogger.AddLog($"{settings.idBanner.displayName} : {bannerZoneId}");
                BotanGameServicesLogger.AddLog($"{settings.idMRec.displayName} : {mrecZoneId}");
                BotanGameServicesLogger.AddLog($"{settings.idInterstitial.displayName} : {interstitialZoneId}");
                BotanGameServicesLogger.AddLog($"{settings.idRewarded.displayName} : {rewardedZoneId}");
        ...
        {
            BotanGameServicesLogger.AddLog("OnConfigurationCompleted called");

            if (zones_ == null || zones_.Count <= 0)
            {
                BotanGameServicesLogger.AddLog("Configuration Failed");
            }
            else
            {
                BotanGameServicesLogger.AddLog("Configuration Succeeded.");
            }
            RequestInterstitial();
            RequestRewarded();
            initialized = true;
        ...
        private void RequestBanner(string bannerID, BannerType bannerType, BannerPosition position)
        {
            ...
            BotanGameServicesLogger.AddLog("Request banner");
            ...
            switch (bannerType)
            {
                ...
                case BannerType.Skyscraper:
                    ...
                    break;
                default:
                    BotanGameServicesLogger.AddLog($"Banner Type: {bannerType} not supported by AdColony, BannerType.Banner will be used");
                    break;
            }
            ...
            switch (position)
            {
                ...
                case BannerPosition.TopRight:
                    ...
                    break;
                default:
                    BotanGameServicesLogger.AddLog($"Banner Position: {position} not supported by AdColony, BannerPosition.Top will be used");
                    break;
            }
        ...
        {
            BotanGameServicesLogger.AddLog("Banner Load Failed ");
        ...
        private void BannerLoaded(AdColonyAdView ad)
        {
            ...
            {
                BotanGameServicesLogger.AddLog("Banner closed before loading");
                bannerAd.DestroyAdView();
            }
        }
        #endregion
        ...
        private void RequestInterstitial()
        {
            ...
            BotanGameServicesLogger.AddLog("Request Interstitial");
        ...
        private void RequestRewarded()
        {
            ...
            BotanGameServicesLogger.AddLog("Request Rewarded");
        ...
        private void OnRewardGranted(string zoneId, bool success, string name, int amount)
        {
            ...
            {
                BotanGameServicesLogger.AddLog(string.Format(" OnRewardGranted called\n\tzoneId: {0}\n\tsuccess: {1}\n\tname: {2}\n\tamount: {3}", zoneId, success, name, amount));
        ...
        {
            BotanGameServicesLogger.AddLog($"OnClosed called, expired: {ad_.Expired}");
        ...
        {
            BotanGameServicesLogger.AddLog("Load Ad Failed");
            ...
            if (zoneID == interstitialZoneId)
            {
                if (currentRetryInterstitial < maxRetryCount)
                {
                    ...
                    BotanGameServicesLogger.AddLog($"Interstitial Failed->Retry {currentRetryInterstitial}");
            ...
            if (zoneID == rewardedZoneId)
            {
                if (currentRetryRewardedVideo < maxRetryCount)
                {
                    ...
                    BotanGameServicesLogger.AddLog($"Rewarded Video Failed->Retry {currentRetryRewardedVideo}");
        ...
        {
            BotanGameServicesLogger.AddLog($"OnRequestInterstitial called id: {ad_.ZoneId}");
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowBuiltInConsentWindow Not supported on {SupportedAdvertisers.AdColony}");
        }
        #endregion
        ...
        {
            BotanGameServicesLogger.AddLog($"OpenDebugWindow Not supported on {SupportedAdvertisers.AdColony}");
        }
        #endregion
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowAppOpen Not supported on {SupportedAdvertisers.AdColony}");
        }
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowRewardedInterstitial Not supported on {SupportedAdvertisers.AdColony}");
        }
        #endregion
    }
}
