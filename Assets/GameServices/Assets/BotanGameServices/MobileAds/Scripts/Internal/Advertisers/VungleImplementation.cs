    ...
    public class VungleImplementation : MonoBehaviour, IAdProvider
    {
        ...
        public void InitializeAds(PlatformSettings platformSettings, UnityAction onInitialized)
        {
            if (initialized == false)
            {
                ...
                BotanGameServicesLogger.AddLog($"{settings.idBanner.displayName} : {bannerPlacementID}");
                BotanGameServicesLogger.AddLog($"{settings.idMRec.displayName} : {mRecPlacementID}");
                BotanGameServicesLogger.AddLog($"{settings.idInterstitial.displayName} : {interstitialPlacementID}");
                BotanGameServicesLogger.AddLog($"{settings.idRewarded.displayName} : {rewardedPlacementId}");
        ...
        public void ShowBanner(string bannerID, BannerPosition position, BannerType bannerType, Vector2Int customSize, Vector2Int customPosition)
        {
            ...
            switch (bannerType)
            {
                ...
                case BannerType.Leaderboard:
                    ...
                    break;
                default:
                    BotanGameServicesLogger.AddLog($"Banner Type: {bannerType} not supported by Vungle, BannerType.Banner will be used");
                    break;
            }
            ...
            switch (position)
            {
                ...
                case BannerPosition.Custom:
                    break;
                default:
                    BotanGameServicesLogger.AddLog($"Banner Position: {position} not supported by Vungle, BannerPosition.Top will be used");
                    break;
            }
        ...
        {
            BotanGameServicesLogger.AddLog(this + " OnAdRewarded " + placementID);
        ...
        private void OnAdEnd(string placementID)
        {
            ...
            BotanGameServicesLogger.AddLog($"OnAdEnd {placementID}");
            ...
            if (placementID == rewardedPlacementId)
            {
                ...
                BotanGameServicesLogger.AddLog($"Load another ad {placementID}");
        ...
        {
            BotanGameServicesLogger.AddLog($"Ad's playable state has been changed! placementID {placementID}.Now: {adPlayable}");
        }


        private void OnPlacementPreparedEvent(string arg1, string arg2)
        {
            BotanGameServicesLogger.AddLog($"OnPlacementPreparedEvent {arg1} {arg2}");
        }


        private void OnErrorEvent(string message)
        {
            BotanGameServicesLogger.AddLog($"OnErrorEvent: {message}");
        ...
        {
            BotanGameServicesLogger.AddLog($"Ad Started: {placementID}");
        }
        ...
        {
            BotanGameServicesLogger.AddLog(obj);
        }
        #endregion
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowBuiltInConsentWindow Not supported on {SupportedAdvertisers.Vungle}");
        }
        #endregion
        ...
        {
            BotanGameServicesLogger.AddLog($"OpenDebugWindow Not supported on {SupportedAdvertisers.Vungle}");
        }
        #endregion
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowAppOpen Not supported on {SupportedAdvertisers.Vungle}");
        }
        ...
        {
            BotanGameServicesLogger.AddLog($"ShowRewardedInterstitial Not supported on {SupportedAdvertisers.Vungle}");
        }
        #endregion
    }
}
