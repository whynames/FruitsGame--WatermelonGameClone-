...
using BotanGameServices.AdsPackage.Local;
...
namespace BotanGameServices.AdsPackage.Editor
{
    public class SettingsWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            mobileAdsSettingsData = EditorUtilities.LoadOrCreateDataAsset<MobileAdsSettingsData>(rootFolder, Local.Constants.DATA_PATH_EDITOR, Local.Constants.DATA_NAME_EDITOR);
        ...
        private void SaveSettings()
        {
            ...
            switch (step)
            {
                case 0:
                    ...
                    EditorUtilities.CreateDataAsset(mobileAdsData, rootFolder, "Resources", Local.Constants.DATA_NAME_RUNTIME);
                    mobileAdsData = Resources.Load<MobileAdsData>(Local.Constants.DATA_NAME_RUNTIME);
        ...
        void AddAdmobPatchDirective()
        {
            ...
            {
                PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_PATCH_ADMOB, false, BuildTargetGroup.Android);
            }
            if (selectediOSAdvertiser == SupportedAdvertisers.Admob)
            {
                PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_PATCH_ADMOB, false, BuildTargetGroup.iOS);
            }
#endif
        }
        ...
        void SetAdmobAppID()
        {
#if BOTANGAMESERVICES_PATCH_ADMOB
            if (selectedAndroidAdvertiser == SupportedAdvertisers.Admob || selectediOSAdvertiser == SupportedAdvertisers.Admob)
            {
                ...
                GoogleMobileAds.Editor.BotanGameServicesAdmobPatch.SetAdmobAppID(andoidAppID, iosAppID);
        ...
        private void SetPreprocessorDirectives()
        {
            if (usePlaymaker)
            {
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, false, BuildTargetGroup.Android);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, false, BuildTargetGroup.iOS);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, false, BuildTargetGroup.WSA);
            }
            else
            {
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, true, BuildTargetGroup.Android);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, true, BuildTargetGroup.iOS);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_PLAYMAKER_SUPPORT, true, BuildTargetGroup.WSA);
            }

            if (useUnityVisualScripting)
            {
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, false, BuildTargetGroup.Android);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, false, BuildTargetGroup.iOS);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, false, BuildTargetGroup.WSA);
            }
            else
            {
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, true, BuildTargetGroup.Android);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, true, BuildTargetGroup.iOS);
                PreprocessorDirective.AddToPlatform(Main.Constants.BOTANGAMESERVICES_UVS_SUPPORT, true, BuildTargetGroup.WSA);
            }

            Debug.Log("ENABLE ATT " + enableATT);
            if (enableATT)
            {
                PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_ATT, false, BuildTargetGroup.iOS);
            }
            else
            {
                PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_ATT, true, BuildTargetGroup.iOS);
            }
                    ...
                    for (int j = 0; j < advertiserSettings[i].platformSettings.Count; j++)
                    {
                        ...
                        else
                        {
                            if (advertiserSettings[i].platformSettings[j].platform == SupportedPlatforms.Android)
                            {
                                ...
                                {
                                    PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_PATCH_ADMOB, true, BuildTargetGroup.Android);
                                }
                            ...
                            if (advertiserSettings[i].platformSettings[j].platform == SupportedPlatforms.iOS)
                            {
                                if (advertiserSettings[i].advertiser == SupportedAdvertisers.Admob)
                                {
                                    PreprocessorDirective.AddToPlatform(SettingsWindowProperties.BOTANGAMESERVICES_PATCH_ADMOB, true, BuildTargetGroup.iOS);
                                }
