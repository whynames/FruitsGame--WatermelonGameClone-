...
namespace BotanGameServices.DailyRewards.Internal
{
    public class CalendarManager : MonoBehaviour
    {
        ...
        public void ShowCalendar()
        {
            if (canvas == null)
            {
                Debug.LogWarning("canvas cannot be null. Go to Tools->BotanGameServices->Daily Rewards to setup");
                return;
            }

            if (popup == null)
            {
                Debug.LogWarning("popup cannot be null. Go to Tools->BotanGameServices->Daily Rewards to setup");
                return;
            }

            if (allDays.Count == 0)
            {
                Debug.LogWarning("You need at least one day. Go to Tools->BotanGameServices->Daily Rewards to setup");
                return;
            }
